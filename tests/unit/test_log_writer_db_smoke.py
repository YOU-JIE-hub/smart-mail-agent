...
def test_log_to_db_inserts_row(tmp_path: Path):
    ...
    try:
        (cnt,) = con.execute("SELECT COUNT(*) FROM emails_log").fetchone(); assert cnt>=2
        row = con.execute("SELECT subject,predicted_label,action FROM emails_log ORDER BY id ASC LIMIT 1").fetchone()
