...
def test_write_pdf_or_txt_pdf_success(tmp_path: Path):
    ...
    p = Path(out); assert p.exists(); assert p.suffix in {".pdf",".txt"}; assert "?" not in p.name
def test_write_pdf_or_txt_txt_fallback(tmp_path: Path, monkeypatch):
    ...
    p = Path(out); assert p.exists() and p.suffix==".txt"; assert p.read_text(encoding="utf-8").strip()=="X"
