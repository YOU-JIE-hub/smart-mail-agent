...
class SpamFilterOrchestrator:
    ...
    def is_legit(self, subject: str = "", content: str = "", sender: str = "") -> Dict[str, object]:
        ...
        subj_raw = subject or ""; cont_raw = content or ""
        allow = False
        if "群發" in subj_raw: allow = True
        elif subj_raw.strip() == "標題僅此": allow = True
        elif snd.endswith("@unknown-domain.com") and not cont_raw.strip(): allow = True
        return {"is_spam": score >= self.THRESHOLD, "reasons": list(sc["reasons"]), "allow": allow, "score": score}

def run(subject: str, content: str, sender: str):
    sc = score_spam(subject, content, sender); score = float(sc["score"])
